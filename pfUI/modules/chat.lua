pfUI:RegisterModule("chat", function ()
  ...
  function _G.FCF_OnUpdate(elapsed)
    ...
    local chatFrame, chatTab, visible

    for i=1, NUM_CHAT_WINDOWS do
      if MouseIsOver(pfUI.chat.left, 10, 10, 10, 10) or
         MouseIsOver(pfUI.chat.right, 10, 10, 10, 10) then
        _G["ChatFrame" .. i .. "Tab"]:SetAlpha(1)
        visible = true
      else
    ...
    if visible then
      pfUI.chat.left.panelTop:SetAlpha(1)
      pfUI.chat.right.panelTop:SetAlpha(1)
    else
      pfUI.chat.left.panelTop:SetAlpha(0)
      pfUI.chat.right.panelTop:SetAlpha(0)
    end
